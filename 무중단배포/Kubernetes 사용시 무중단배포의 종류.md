...
개발자들은 서비스의 일부 기능이 장애가 나지 않도록 인스턴스 종료 직전 가장 마지막으로 들어온 요청 까지만 처리하고 그 이후의 요청부터는 받지 않는 우아한 종료에 관련된 이야기를 많이 들었고 다중 인스턴스로 운영되는 개발환경에 이런 점들을 적용해오는 경우가 많다. 오늘 이야기는 이런 소프트웨어적인 처리에서 조금 더 나아가서 인프라 레벨에서의 이야기이다. <br>

전체적인 요지는 **"서비스를 운영하는 입장에서 무중단 배포를 할 때 어떤 종류의 전략을 취하는가?"**에 대한 이야기이다.<br>

항상 이런 문서를 정리하다보면, **"개발자 레벨에서 투머치하게 이해하려고 하는 건가?"** 하고 나 자신을 의심할 때가 자주 있다. 그런데 이 내용은 개발자도 일부 알아야 한다. 데브옵스나 인프라 팀 계열의 엔지니어는 이런 개념을 토대로 자원을 관리하고 비용을 관리하고 계획을 세우는 분들이라면, 개발자인 우리는 이런 것들을 아무것도 없는 상태에서 어느 정도는 돌아갈 수 있도록 기틀을 만들수 있어야 한다... 라는 생각이 들었다.<br>
...
- 3) `v2` 버전의 서비스가 정상적으로 잘 띄워졌고, 잘 동작하는지를 체크 후 `v2` 가 정상이라고 판단된다면 트래픽이 `v2` 으로 향하도록 바꿔준다. 그리고 `v1` 으로 배포되어 있는 Pod 3기를 종료시킨다.
    - 현재 상태
        - `v1` : Pod 3개 → Pod 0개
